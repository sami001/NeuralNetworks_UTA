        ...
        for i in range(self.nIter):
           for j in range(self.nDelay+1, self.trainingSize):
              ...
              if((j+1) % self.batchSize == 0):
                  ...
                  abs_max_price = -1
                  abs_max_volume = -1
                  sum_of_square_price = 0
                  sum_of_square_volume = 0
                  '''
                  ...
                  for k in range(self.nDelay + 1, self.testSize):
                      ...
                      t = np.append(self.training_data[k, 0], self.training_data[k, 1]).reshape((-1, 1))
                      a = np.dot(self.weight, p) + self.bias
                      '''
                      if (abs(t[0,0] - a[0,0]) > abs_max_price):
                          abs_max_price = abs(t[0,0] - a[0,0])
                      if (abs(t[1,0] - a[1,0]) > abs_max_volume):
                          abs_max_volume = abs(t[1,0] - a[1,0])
                      sum_of_square_price += ((t[0,0] - a[0,0]) ** 2)
                      sum_of_square_volume += ((t[1,0] - a[1,0]) ** 2)
                      '''
                  ...
                  '''
                  mse_price[batch] = sum_of_square_price/(self.testSize-(self.nDelay + 1))
                  mae_price[batch] = abs_max_price
                  mse_volume[batch] = sum_of_square_volume/(self.testSize-(self.nDelay + 1))
                  mae_volume[batch] = abs_max_volume
                  '''
                  batch += 1
